...
`config/herdr-presentation-spaces` also accepts `project`, a third value mutually exclusive with `off` and `on`: instead of a disposable one-task workspace, every task for one project shares that project's own durable workspace, keyed by the project's registry name - the same key `bin/fm-project-mode.sh` and `bin/fm-spawn.sh`'s own standing-mode lookup already use - and scoped to this home exactly like the per-home workspace above, so a project worked by both the primary and a secondmate gets two independent workspaces.
...
That persisted, live-verified record is the only path to placement; unlike the per-home lookup above, project grouping never searches by label, so a pre-existing workspace that happens to carry the same `proj-<project-key>` label - a captain's own scratch space, or another home's leftover - is never adopted.
