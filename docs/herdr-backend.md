...
`config/herdr-presentation-spaces` also accepts `project`, a third value mutually exclusive with `off` and `on`: instead of a disposable one-task workspace, every task for one project shares that project's own durable workspace, identified by a collision-resistant project key - the project's registry name (the same name `bin/fm-project-mode.sh` and `bin/fm-spawn.sh`'s own standing-mode lookup already use) followed by a short hash of the project's full absolute path (`fm_backend_herdr_project_key_for_path`) - and scoped to this home exactly like the per-home workspace above; two differently-located projects that happen to share a registry name (e.g. `~/oss/frontend` and `~/work/frontend`) never collide into the same workspace.
When a registered secondmate's `projects:` list (`data/secondmates.md`) names the project being spawned into, placement skips this project's own dedicated workspace entirely and reuses that secondmate's existing home-labeled workspace (`2ndmate-<secondmate-id>`) instead, so the same project worked by both the primary and that secondmate converges on one visible space rather than getting two independent ones.
That reuse is a plain live label lookup against the current session - the exact mechanism the per-home workspace above already uses to resolve a home's own workspace, given the secondmate's id in place of the launching process's own `FM_HOME` - never a persisted record, so there is nothing to self-heal or go stale: a remote secondmate's workspace lives on a different herdr install entirely, and a local secondmate that has not yet spawned a task of its own has no workspace at all, so either one simply finds no live match and falls back to this project's own dedicated workspace below with no special-casing and no failed spawn.
A task placed in a secondmate's workspace this way remains entirely the launching home's own task: its metadata, endpoint, cleanup, and lock scope stay owned by whichever home's `state/` recorded it, exactly like every other task in that home's own workspace, and the secondmate gains no record of it and no cleanup or lock responsibility for it.
Firstmate persists this project's own dedicated-workspace binding in `state/proj-<project-key>.herdr-workspace` and verifies it live - matching home, matching session, and the workspace's own continued presence in the session - before any spawn trusts it.
...
That persisted, live-verified record is the only path to placement in this project's own dedicated workspace; unlike the per-home lookup above and the secondmate-routed reuse above, this fallback path never searches by label, so a pre-existing workspace that happens to carry the same visible label - a captain's own scratch space, or another home's leftover - is never adopted.
...
The secondmate-routed reuse above is not part of that gate: it is shaped exactly like the per-home lookup's own unconditional label search, reusing a workspace someone else's home already maintains rather than creating or draining a project-owned one, so it carries the same unconditional exposure the flat layout already accepts on every release.
...
`tests/fm-backend-herdr-project-secondmate-e2e.test.sh` covers the secondmate-routed reuse above: a project a registered secondmate covers lands a primary-launched task in that secondmate's own workspace, a project with no covering secondmate keeps the ordinary project-workspace behavior unchanged, and the routed task's cleanup and lock scope stay with the primary that owns its metadata.
