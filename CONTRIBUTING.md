...
`.no-mistakes.yaml` publishes test evidence to the orphan `no-mistakes/evidence` branch, which shares no history with code branches, chains the gate's lint command from `bin/fm-lint.sh` (matching the Linux CI lint job) with the non-bypassable commit-attribution backstop, and pins its test command to `bin/fm-test-run.sh --changed --exclude-family real-herdr-gated`.
See [`docs/architecture.md`](docs/architecture.md)'s "No-mistakes gate authority boundary" section for that trust-boundary's mechanics.
